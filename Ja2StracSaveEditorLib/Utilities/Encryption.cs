...
{
    private const int BaseNumberOfRotationArrays = 19;
    ...
    public static int CalcJa2EncryptionSet(SAVED_GAME_HEADER h)
    {
        ...
        */
        ...
        set %= 10;
        set += (int)(h.uiDay / 10);
        set %= BaseNumberOfRotationArrays;

        GAME_OPTIONS o = h.sInitialGameOptions;
        if (o.fGunNut) set += BaseNumberOfRotationArrays * 6;
        if (o.fSciFi) set += BaseNumberOfRotationArrays * 3;
        ...
        switch (o.ubDifficultyLevel)
        {
            ...
            case (byte)DifficultyLevel.DIF_LEVEL_HARD:
                set += BaseNumberOfRotationArrays * 2;
                break;
        }

        Debug.Assert(set < BaseNumberOfRotationArrays * 12);
    ...
    public static uint SoldierProfileChecksum(MercProfile p)
    {
        uint sum = 1;
    ...
    public static uint MercChecksum(Soldier s)
    {
        uint sum = 1;

        sum += (uint)(1 + s.bLife);
        sum *= (uint)(1 + s.bLifeMax);
        sum += (uint)(1 + s.bAgility);
        sum *= (uint)(1 + s.bDexterity);
        sum += (uint)(1 + s.bStrength);
        sum *= (uint)(1 + s.bMarksmanship);
        sum += (uint)(1 + s.bMedical);
        sum *= (uint)(1 + s.bMechanical);
        sum += (uint)(1 + s.bExplosive);
        sum *= (uint)(1 + s.bExpLevel);
        sum += (uint)(1 + s.ubProfile);

        foreach (var i in s.inv)
        {
            //sum += i.usItem;
            sum += i.usItemCheckSum;
            sum += i.ubNumberOfObjects;

            if (i.usItem != i.usItemCheckSum)
            {
                Console.WriteLine($"Unknown item id = {i.usItemCheckSum}");
            }
        }
    ...
    public static byte[] NewJa2EncryptedFileWrite(int setId, byte[] data, uint uiBytesToWrite)
    {
        if (data == null) throw new ArgumentNullException(nameof(data));
        if (uiBytesToWrite > data.Length)
            throw new ArgumentOutOfRangeException(nameof(uiBytesToWrite), "uiBytesToWrite exceeds data length.");
