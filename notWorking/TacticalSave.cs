...
public static class TacticalSave
{
    private const int NewRotationArraySize = 49;
    private const int RotationArraySize = 46;

    // ReSharper disable once InconsistentNaming
    public const int BASE_NUMBER_OF_ROTATION_ARRAYS = 19;

    // Массив для новой системы шифрования (аналогично оригинальному C++ коду)
    private static readonly byte[][] GEncryptionArray =
    {
        // Здесь должен быть полный массив g_encryption_array из оригинального кода
        // Для краткости показана только структура
    ...
    // Старый массив для шифрования
    private static readonly byte[] UbRotationArray =
    {
        132, 235, 125, 99, 15, 220, 140, 89, 205, 132, 254, 144, 217, 78, 156, 58, 215, 76, 163, 187, 55, 49, 65, 48,
        156, 140, 201, 68, 184, 13, 45, 69, 102, 185, 122, 225, 23, 250, 160, 220, 114, 240, 64, 175, 057, 233
    };

    /*public static uint GuiJa2EncryptionSet = 0;

    private static byte[] GetRotationArray()
    {
        if (GuiJa2EncryptionSet >= GEncryptionArray.Length)
        {
            throw new ArgumentOutOfRangeException(nameof(GuiJa2EncryptionSet));
        }

        return GEncryptionArray[GuiJa2EncryptionSet]; // Используем первый массив из набора
    }
    */

    public static int GetRotationArrayLenth()
    {
        return GEncryptionArray.Length;
    }

    private static byte[] GetRotationArray(int setId)
    {
        if (setId >= GEncryptionArray.Length)
        {
            throw new ArgumentOutOfRangeException(nameof(setId));
        }

        return GEncryptionArray[setId];
    }

    /* Новая функция чтения с шифрованием
    public static void NewJa2EncryptedFileRead(DataReader reader, byte[] pDest, uint uiBytesToRead)
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (pDest == null) throw new ArgumentNullException(nameof(pDest));

        // Читаем зашифрованные данные
        byte[] encryptedData = new byte[uiBytesToRead];
        reader.ReadBytes(encryptedData, (int)uiBytesToRead);
        if (encryptedData.Length != uiBytesToRead)
        {
            throw new EndOfStreamException("Could not read the requested number of bytes");
        }

        byte[] pubRotationArray = GetRotationArray();
        byte ubArrayIndex = 0;
        byte ubLastByte = 0;

        for (int i = 0; i < uiBytesToRead; i++)
        {
            byte ubLastByteForNextLoop = encryptedData[i];
            pDest[i] = (byte)(encryptedData[i] - ubLastByte - pubRotationArray[ubArrayIndex]);

            if (++ubArrayIndex >= NewRotationArraySize)
                ubArrayIndex = 0;
    ...
    public static int CalcJA2EncryptionSet(SAVED_GAME_HEADER h)
    {
        var set = h.iCurrentBalance;
    ...
    /*
    public static byte[] NewJa2EncryptedFileRead(byte[] encryptedData, uint uiBytesToRead)
    {
        var pDest = new byte[uiBytesToRead];
        // Читаем зашифрованные данные
        if (encryptedData.Length != uiBytesToRead)
        {
            throw new EndOfStreamException("Could not read the requested number of bytes");
    ...
    */
    ...
    public static byte[] NewJa2EncryptedFileRead(int setId, byte[] encryptedData, uint uiBytesToRead)
    {
        var pDest = new byte[uiBytesToRead];
        // Читаем зашифрованные данные
        if (encryptedData.Length != uiBytesToRead)
        ...
        byte[] pubRotationArray = GetRotationArray(setId);
        byte ubArrayIndex = 0;
        byte ubLastByte = 0;
    ...
    /*
    public static void NewJA2EncryptedFileRead_(Stream f, byte[] pDest, uint uiBytesToRead)
    {
        // Считываем байты из потока
        int bytesRead = f.Read(pDest, 0, (int)uiBytesToRead);
        if (bytesRead != uiBytesToRead)
            throw new EndOfStreamException("Не удалось прочитать нужное количество байт из потока.");

        byte[] pubRotationArray = GetRotationArray(); // Предполагается, что эта функция уже реализована
        byte ubArrayIndex = 0;
        byte ubLastByte = 0;

        for (int i = 0; i < uiBytesToRead; ++i)
        {
            byte ubLastByteForNextLoop = pDest[i];
            pDest[i] = (byte)(pDest[i] - ubLastByte - pubRotationArray[ubArrayIndex]);
            ubArrayIndex++;
            if (ubArrayIndex >= NewRotationArraySize) ubArrayIndex = 0;
            ubLastByte = ubLastByteForNextLoop;
        }
    }*/

    // Новая функция записи с шифрованием
    public static void NewJa2EncryptedFileWrite(int setId, BinaryWriter writer, byte[] data, uint uiBytesToWrite)
    {
        if (writer == null) throw new ArgumentNullException(nameof(writer));
        if (data == null) throw new ArgumentNullException(nameof(data));

        byte[] buf = new byte[uiBytesToWrite];
        byte[] pubRotationArray = GetRotationArray(setId);
        byte ubArrayIndex = 0;
        byte lastByte = 0;

        for (int i = 0; i < uiBytesToWrite; i++)
        {
            buf[i] = (byte)(data[i] + lastByte + pubRotationArray[ubArrayIndex]);
            lastByte = buf[i];
    ...
    // Старая функция чтения с шифрованием
    public static void Ja2EncryptedFileRead(DataReader reader, byte[] pDest, uint uiBytesToRead)
    {
        if (reader == null) throw new ArgumentNullException(nameof(reader));
        if (pDest == null) throw new ArgumentNullException(nameof(pDest));
        ...
        {
            byte ubLastByteForNextLoop = encryptedData[i];
            pDest[i] = (byte)(encryptedData[i] - ubLastByte - UbRotationArray[ubArrayIndex]);

            if (++ubArrayIndex >= RotationArraySize)
                ubArrayIndex = 0;
